<template>
	<div class="p-4 text-gray-200 ">
		...
		<div class="mt-4">
			...
			<p class="text-gray-400 text-sm block pb-3 mb-2 border-b border-gray-800">
			...
			<div class="space-y-4">
				<div v-for="element, idx in model.spec" class="bg-gray-900 rounded-md p-4">
					...
					<o-field label="Help message" message="Used to inform users about the field and how it will be used. This message is actually what it will look like so just type something you'd want to see here!" class="mt-2 flex-grow">
						<input v-model="element.help" class="w-full rounded-md bg-gray-800 text-sm px-2 py-2 border-none border-gray-700 focus:outline-none focus:border-primary-900 focus:ring-primary-600 focus:ring-1"/>
					</o-field>
					...
					<div v-if="element.type == 'relationship'" class="mt-2">
						<div class="flex flex-row items-center mb-2">
							<b class="text-gray-200 text-sm flex-grow">Relationship options</b>
						</div>
						<o-autocomplete
							v-model="element.args.model"
							placeholder="e.g. Posts, Authors, etc."
							:open-on-focus="true"
							:data="filtered_models(element)"
							field="id"
					...
					<div class="mt-2" v-if="model.id == 'new' && ['string', 'dropdown', 'tag', 'relationship'].includes(element.type)">
			...
			filtered_models(elm) {
				return this.models.filter(model => {
					return (
						model.id
						.toLowerCase()
						.indexOf((elm.args.model || '').toLowerCase()) >= 0
					)
				})
			},
			move_key(idx, move) {
